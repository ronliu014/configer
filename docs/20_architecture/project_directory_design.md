...
- source 样例与 target 产物分离，避免误覆盖源表。
...
`source/` 保存真实或接近真实的输入配表样例，用于规则分析和测试对比。运行时的 `sourceRoot` 由用户选择，仓库内 `source/` 不能作为工具固定依赖。不要把 target 产物、备份、changelog、构建产物放入此目录。

`src/app/` 保存应用外壳、路由、侧边栏、全局状态入口。它负责组合模块，不写具体业务规则。

`src/core/` 保存平台能力，例如 Excel 解析、4 行表头协议、schema 合并、对账引擎、target 输出器、备份、changelog。
...
- Excel 输出能力只能位于 `core`，业务模块只提交结构化变更请求。

## 生成文件规则

备份、changelog、临时导出、构建产物应放入用户选择的 `targetRoot` 或明确的构建输出目录，并默认不纳入版本库。`targetRoot` 中目标文件按源文件相对 `sourceRoot` 的路径 1:1 镜像生成；覆盖已有目标文件前必须先生成备份和变更记录。
