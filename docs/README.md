...
docs/
  ...
  40_modules/        模块级：equip、item、language 等业务模块说明
  90_reference/      参考级：Excel 协议、源表清单、外部流程说明
  equip/doc/         现有 equip 领域分析资料，暂不搬迁
```
...
3. 做装备 MVP 前读 [equip 模块入口](40_modules/equip/README.md)，再进入 `docs/equip/doc/` 的详细资料。
4. 改 Excel 读写、对账、回写前必须读 `docs/equip/doc/12_装备配置工具设计规范.md` 与 `docs/equip/doc/15_工具开发实施指南.md`。
...
4. `docs/40_modules/` 中的模块入口和模块摘要。
5. `docs/equip/doc/` 中的详细分析资料。

如果详细资料与上层决策冲突，不要静默选择其一；应更新上层文档或在模块入口记录差异。
